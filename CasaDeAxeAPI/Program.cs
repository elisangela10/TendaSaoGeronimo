...
using (var scope = app.Services.CreateScope())
{
    ...
    {
        logger.LogError(ex, "Falha ao conectar no banco de dados durante o startup.");
        var detalhe = ex is NpgsqlException ? ex.Message : ex.InnerException?.Message ?? ex.Message;
        throw new InvalidOperationException($"Não foi possível conectar ao banco de dados. Detalhe: {detalhe}", ex);
    }
